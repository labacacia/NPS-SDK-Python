...
- **NWP — `SubscribeFrame` CR-0006** (Breaking rewrite): Wire format replaced with CR-0006 formal spec — `subscription_id` (UUID v4), `filter` (`dict?`), `heartbeat_interval_ms` (`int?`), `max_events` (`int?`), `cursor` (`str?`). **Wire breaking change vs alpha.8–10.**
- **NOP — AlignStream ack/NAK**: `AlignStreamFrame` gains `ack_seq` and `nak_seq` (`int?`) for NOP v0.6 sliding-window acknowledgement.
- **NOP — Saga compensation**: `TaskFrame.compensation_policy`; `DelegateFrame.target_cluster_anchor`; `AggregateStrategy.WEIGHTED_FIRST_K` / `MERGE_ALL`.
- **NDP — `GraphFrame` §5** (Breaking rewrite): `NdpGraphNode`, `NdpGraphEdge` dataclasses; `GraphFrame` with `graph_id`, `nodes`, `edges`, `ttl`, `metadata`. Max 256 nodes / 1024 edges.
- **NDP — `SecurityProfile`**: `LOCAL_DEV` / `ORG_PRIVATE` / `PUBLIC_FEDERATED` constants.
- **NIP — `IdentFrame.ocsp_staple`**: base64url DER OCSP response field; `IdentReputationPolicyHint` dataclass.
...
- **NOP — Saga compensation**: `DagNode` with `compensate_action` / `compensate_params_mapping`; `TaskState.COMPENSATING` / `COMPENSATED`; `CompensationPolicy` enum.
- **NDP — `SecurityProfile`**: `LOCAL_DEV` / `ORG_PRIVATE` / `PUBLIC_FEDERATED` constants.
- **NIP — `IdentReputationPolicyHint`**: Reputation policy hint dataclass; `IdentMetadata` wrapper.
...
- **NWP — `SubscribeFrame` (0x12)**: Initial `SubscribeFrame` dataclass (pre-CR-0006 format — replaced in alpha.11).
- **NIP — `IdentFrame` assurance improvements**: Structured assurance-level extraction per NPS-RFC-0003 draft.
...
- **`nps_sdk.nip.x509.oids` — IANA PEN 65715 (Breaking, CR-0004)**: `NpsX509Oids.LAB_ACACIA_PEN_ARC` updated from `"1.3.6.1.4.1.99999"` to `"1.3.6.1.4.1.65715"`. Certificates issued under the provisional arc must be revoked and re-issued.
